...
/**
 * Collection of methods to handle MADkitchen lookup tables.
 *
 * @package     Database
 * @subpackage  Handler
 * @since       0.1
...
class Lookup {
    ...
    /**
     * Check if query is simple one which can be addressed to lookup tables.
     *
     * Simple query contains only column search keys, which shall be all included in target table.
     *
     * @param string $class The target MADkitchen module class
     * @param string $table The target table name
    ...
    public static function is_simple_query($class, $table, $query) {
        return !empty($query) &&
                !empty($table_cols = \MADkitchen\Database\Handler::get_tables_data($class, $table)['columns']) &&
                count($item = array_intersect_assoc(array_keys($table_cols), array_keys($query))) === count($query);
    }
        ...
        if (self::is_lookup_table($class, $table)) {
            if (!isset(self::$lookup_tables[$class][$table])) {
                self::$lookup_tables[$class][$table] = \MADkitchen\Modules\Handler::$active_modules[$class]['class']->query($table, [
                            ...
                            'order' => 'ASC',
                            'number' => 0, //no limit to retrieved items
                                ]
                        )->items;
            }
